...
def svhn_dataloaders(
    ...
    if class_to_replace is not None and indexes_to_replace:
        raise ValueError(
...
def cifar100_dataloaders(
    ...
    for i in range(max(train_set.targets) + 1):
        ...
        valid_idx.append(
            val_rng.choice(class_idx, int(0.1 * len(class_idx)), replace=False)
        )
    valid_idx = np.hstack(valid_idx)
    print(valid_idx)
    train_set_copy = copy.deepcopy(train_set)
...
def cifar10_dataloaders(
    ...
    if class_to_replace is not None:
        ...
        if num_indexes_to_replace is None or num_indexes_to_replace == 4500:
            test_set.data = test_set.data[test_set.targets !=\
                    class_to_replace]
    ...
    if indexes_to_replace:
        ...
        '''
        test_prime = copy.deepcopy(test_set)
        test_prime_idx = []
        for i in range(max(test_set.targets) + 1):
            class_idx = np.where(test_set.targets == i)[0]
            test_prime_idx.append(
                rng.choice(class_idx, int(0.8 * len(class_idx)), replace=False)
            )
        test_prime_idx = np.hstack(test_prime_idx)
        test_set_copy = copy.deepcopy(test_set)

        test_prime.data = test_set_copy.data[test_prime_idx]
        test_prime.targets = test_set_copy.targets[test_prime_idx]

        test_idx = list(set(range(len(test_set))) - set(test_prime_idx))

        test_set.data = test_set_copy.data[test_idx]
        test_set.targets = test_set_copy.targets[test_idx]
        '''
    ...
    val_loader = DataLoader(
        valid_set,
        batch_size=batch_size,
        shuffle=True, # SHUFFLE TRUE BECAUSE THE MODEL USES BATCH STATS IN TRAIN() MODE TO DO INFERENCE. OTHERWISE, THE INFERENCE PERFORMANCE IS BAD
        worker_init_fn=_init_fn if seed is not None else None,
        **loader_args,
    )
    test_loader = DataLoader(
...
def replace_indexes(
    dataset: torch.utils.data.Dataset, indexes, seed=0, only_mark: bool = False, choose:bool = False):
    rng = np.random.RandomState(seed)
    if choose:
        indexes = rng.choice(list(set(range(len(dataset)))), size=int(0.1*len(dataset)), replace=False)
        print(f'CHOOSING INDICES RANDOMLY. NUMBER OF FORGET SAMPLES = {len(indexes)}')
    if not only_mark:
