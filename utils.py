...
'''
class GanAttacker(nn.Module):
    def __init__(self, nclasses):
        super(GanAttacker, self).__init__()
        self.nn = nn.Sequential(
                nn.Linear(nclasses, 512),
...
class GanAttacker(nn.Module):
    def __init__(self, nclasses=10):
        ...
        dropout = 0.2
        output_dim = 2
        self.fc1 = nn.Linear(nclasses, 512)
...
def setup_model_dataset(args):
    if args.dataset == "cifar10":
        ...
        train_full_loader, _, _ = cifar10_dataloaders(
        ...
        marked_loader, val_loader, test_loader = cifar10_dataloaders(
            batch_size=args.batch_size,
            data_dir=args.data,
            num_workers=args.workers,
        ...
        if args.unlearn == "Gan":
            Attacker = GanAttacker(classes*3)
            models = (model, Attacker)
        else:
            models = (model, model)
        #print(model)
        return models, train_full_loader, val_loader, test_loader, marked_loader
